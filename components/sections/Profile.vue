<template>
  <CommonResponsiveContainer class="profile bg-indigo-50">
    <div class="profile__cover flex justify-center px-4 mb-6">
      <div class="relative">
        <div class="cover__img rounded-full overflow-hidden outline outline-white outline-offset-2">
          ...
          <span class="absolute bg-white rounded-full py-1 px-2 absolute text-md md:text-lg">I'm Kevin G 👋</span>
        </div>
      </div>
    </div>
    ...
    <div class="profile__controls flex justify-center px-4">
      <CommonButtom target="_blank" href="https://www.linkedin.com/in/kevocde" type="link" class="bg-indigo-400 hover:bg-indigo-500 hover:animation-shake">Contact Me ↗</CommonButtom>
    </div>
    <div class="profile__chat fixed right-8 bottom-8">
      <template v-if="!isChatVisible">
        <div class="chat__toggle flex gap-2 items-center bg-emerald-50 rounded-full p-3 shadow-lg animate-bounce">
          <p class="text-md text-gray px-4 text-emerald-500">Hi, <strong>I'm Kevin AI</strong>, ready to answer your questions about Kevin</p>
          <a class="w-16 h-16 rounded-full bg-emerald-400 flex items-center justify-center cursor-pointer hover:bg-emerald-500  shadow-lg text-2xl text-white" @click="toggleChat"><i class="lni lni-message-2"></i></a>
        </div>
      </template>
      <template v-else>
